...
cells.append('''function rmse_r2_pred(fmu, y)
	pmean = vec(fmu)
...
cells.append('''function train_bp!(nn, Xmat, y32; epochs=400, lr=1f-2, seed=1)
	Random.seed!(seed)
...
cells.append('''function additive_update(mu_A, Lambda_A, sigma_A, theta_init, re; tau=1.0, epochs=400, lr=1f-2, seed=2)
...
cells.append('''function online_round(mu_prev, Lambda_prev, sigma_prev, theta_prev, re_prev, Xk_mat, yk32; tau=1.0, epochs=400, lr=1f-2, seed=1)
...
cells.append('''function run_sequence(; tau=1.0, seed_offset=10)
	nn0 = make_nn()
...
cells.append('''plot(0:K_rounds, seq_no_forget.avg_sd, marker=:circle, linewidth=2, label="τ=1 (no forgetting)",
...
write_notebook(os.path.join(REPO_DIR, "notebooks/04_laplace_additive_updating.jl"), cells)
